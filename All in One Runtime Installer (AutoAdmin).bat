...
:: OTHER RUNTIMES (FIXED + SAFE EXECUTION)
IF EXIST "Other-Runtimes\Other-Runtime-Installer.bat" (
    CALL "Other-Runtimes\Other-Runtime-Installer.bat"
) ELSE (
    echo.
    echo WARNING: Other-Runtime-Installer.bat is missing.
    echo Skipping Other Runtimes step...
    echo.
)

echo Successfully installed all Microsoft's Visual C Runtimes, .NET runtimes, and DirectX!
echo.
